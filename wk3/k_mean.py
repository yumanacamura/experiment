...
clnr_vec,mp3_vec,test_vec = [get_vec('../k_mean/{0}.vec=tfidf.txt'.format(name)) for name in suf]
...
for vec in test_vec:
    teacher_vec.sort(key = (lambda x:-cos_sim(x[0],vec)))
    #print(cos_sim(teacher_vec[0][0],vec))
    if len(list(filter(lambda x:x[1]=='cleaner',teacher_vec[:k]))) > k/2:
        ans.append('cleaner')
    else:
